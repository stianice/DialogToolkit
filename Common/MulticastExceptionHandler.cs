...
public readonly struct MulticastExceptionHandler
{
    ...
    public bool CanHandle(Exception exception) => GetDelegate(exception.GetType()) is not null;
    ...
    public async Task HandleAsync(Exception exception, object? parameter = null)
    {
        ...
        var invokeMethod =
            multicastDelegate.GetType().GetMethod("Invoke")
        ...
        var arguments = parameters.Length switch
        {
            ...
            1 => typeof(Exception).IsAssignableFrom(parameters[0].ParameterType)
