...
using Mvvm.DialogToolkit.Common;
...
public readonly struct DialogCallback
{
    ...
    public Task Invoke(Exception ex) => Invoke(new DialogResult { Exception = ex });
    ...
    public async Task Invoke(IDialogResult result)
    {
        if (
            _empty
            || (
                result.Exception is DialogException
                && result.Exception.Message == DialogException.CanCloseIsFalse
            )
        )
        {
            return;
    ...
    /// </summary>
    public static DialogCallback Empty => new(true);
