...
{
    private readonly MulticastDelegate? _callback = null;
    ...
    public void Invoke(ButtonResult result) => Invoke(new DialogResult(result));
    ...
    public void Invoke(IDialogParameters parameters, ButtonResult result = ButtonResult.None) =>
        Invoke(new DialogResult { Parameters = parameters, Result = result });
        ...
        switch (_callback)
        {
            ...
            default:
                throw new InvalidOperationException(
                    "The DialogCloseCallback has not been properly initialized. This must be initialized by the DialogService, and should not be set by user code."
                );
        }
    }
}
